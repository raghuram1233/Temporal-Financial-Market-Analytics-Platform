...
class TestCostBasis:
    ...
    def test_second_buy_produces_weighted_average(self, clean_db, make_user, asset_id, set_price):
    ...
    def test_selling_does_not_change_the_average(self, clean_db, make_user, asset_id, set_price):
    ...
    def test_selling_everything_closes_the_position(self, clean_db, make_user, asset_id):
        user = make_user()
        buy(clean_db, user, asset_id, 10)
...
class TestRealizedPnl:
    def test_profit_is_booked_against_average_cost(self, clean_db, make_user, asset_id, set_price):
    ...
    def test_loss_is_recorded_as_negative(self, clean_db, make_user, asset_id, set_price):
        user = make_user()
        buy(clean_db, user, asset_id, 10)
...
class TestBiTemporalModel:
    ...
    def test_exactly_one_current_row_per_holding(self, clean_db, make_user, asset_id, set_price):
        user = make_user()
        buy(clean_db, user, asset_id, 10)
        set_price(asset_id, 120)
    ...
    def test_superseded_versions_are_retained(self, clean_db, make_user, asset_id, set_price):
...
class TestPortfolioSummaryView:
    def test_unrealized_pl_tracks_the_current_price(self, clean_db, make_user, asset_id, set_price):
        user = make_user()
    ...
    def test_closed_positions_leave_the_summary(self, clean_db, make_user, asset_id):
        ...
        rows = clean_db.query_all("SELECT * FROM portfolio_summary WHERE user_id = %s", (user,))
        assert rows == []
