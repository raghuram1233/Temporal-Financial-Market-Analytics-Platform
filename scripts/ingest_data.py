    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (asset_id, price, datetime.now(UTC), source),
            )
            conn.commit()
...
def has_historical_data(asset_id, days=365):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT MIN(time) FROM market_data WHERE asset_id = %s", (asset_id,))
            ...
            if oldest_record.tzinfo is None:
                oldest_record = oldest_record.replace(tzinfo=UTC)
        ...
        try:
            ...
            if not real_hist.empty:
                for timestamp, row in real_hist.iterrows():
                    ...
                    if first_real_price is None:
                        first_real_price = price
            ...
            start_date = datetime.now(UTC) - timedelta(days=real_days + 1)
            ...
            for i in range(sim_days):
                ...
                if current_sim_price <= 0:
                    current_sim_price = 1.0

                insert_market_price_historical(
                    asset_id, current_sim_price, sim_time, "simulated_backfill"
                )
    ...
    while True:
        for asset_id, symbol, asset_type in assets:
            try:
                ...
                print(
                    f"[{datetime.now(UTC).strftime('%H:%M:%S')}] {symbol}: ₹{price:,.2f} ({source})"
                )
